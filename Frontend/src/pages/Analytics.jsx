...
import {
  ...
  PolarAngleAxis, AreaChart, Area, XAxis, YAxis, Tooltip, CartesianGrid
} from 'recharts';
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="rounded-xl px-4 py-3 shadow-2xl" style={{ backgroundColor: 'var(--bg-tertiary)', border: `1px solid var(--border-color)` }}>
      ...
      {payload.map(p => (
        <p key={p.name} className="text-sm font-medium" style={{ color: p.stroke }}>
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      transactions.forEach(tx => {
        ...
        if (txMonth === currentMonth && txYear === currentYear && tx.type === 'expense') {
          const category = tx.category || 'Other';
          categoryMap[category] = (categoryMap[category] || 0) + (tx.amount || 0);
        }
      });
      ...
      setRadarData(categoryArray.map(c => ({ subject: c.name, value: c.value })));
  ...
  return (
    <div className="p-8 max-w-screen-2xl mx-auto flex flex-col gap-6">
      <div className="fade-up">
        ...
        <p className="text-xs font-mono mt-0.5" style={{ color: 'var(--text-muted)' }}>Deep dive into your spending patterns</p>
      </div>

      {/* Savings trend */}
      <div className="card p-6 fade-up" style={{ animationDelay: '100ms' }}>
        <h3 className="font-display text-lg font-semibold mb-1" style={{ color: 'var(--text-primary)' }}>Savings Over Time</h3>
        <p className="text-xs font-mono mb-6" style={{ color: 'var(--text-muted)' }}>Income minus expenses per month</p>
        {monthlyData.length > 0 ? (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={monthlyData.map(m => ({ ...m, savings: m.income - m.expenses }))}>
              <defs>
              ...
              <YAxis tick={{ fill: 'rgba(255,255,255,0.3)', fontSize: 11, fontFamily: 'DM Mono' }} axisLine={false} tickLine={false} tickFormatter={v => `₹${v/1000}k`} />
              <Tooltip content={<CustomTooltip />} />
              <Area type="monotone" dataKey="savings" name="Savings" stroke="#6dc7a4" strokeWidth={2} fill="url(#savGrad)" dot={false} activeDot={{ r: 4, fill: '#6dc7a4' }} />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="card p-6 fade-up" style={{ animationDelay: '200ms' }}>
          <h3 className="font-display text-lg font-semibold mb-1" style={{ color: 'var(--text-primary)' }}>Spend Distribution</h3>
          <p className="text-xs font-mono mb-4" style={{ color: 'var(--text-muted)' }}>Relative spending by category</p>
          <ResponsiveContainer width="100%" height={260}>
            <RadarChart data={radarData}>
              ...
              <Radar name="Spending" dataKey="value" stroke="#d0b880" fill="#d0b880" fillOpacity={0.15} strokeWidth={2} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card p-6 fade-up" style={{ animationDelay: '300ms' }}>
          <h3 className="font-display text-lg font-semibold text-white mb-1">Top Spending Categories</h3>
          <p className="text-xs font-mono mb-5" style={{ color: 'var(--text-muted)' }}>Ranked by total spend this month</p>
                ...
                <div key={c.name} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>{formatCurrency(c.value, true)}</span>
                    </div>
