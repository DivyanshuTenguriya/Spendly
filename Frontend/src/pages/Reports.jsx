  ...
  return (
    <div className="p-8 max-w-screen-2xl mx-auto flex flex-col gap-6">
      <div className="flex items-center justify-between fade-up">
        <div>
          <h2 className="font-display text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Reports</h2>
          <p className="text-xs font-mono mt-0.5" style={{ color: 'var(--text-muted)' }}>Visualised summaries of your finances</p>
        </div>
      </div>

      {categoryData.length === 0 ? (
        <div className="card p-6 fade-up">
          <p className="text-center py-12" style={{ color: 'var(--text-muted)' }}>No data available. Add some transactions to see reports.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {/* Category full pie */}
          <div className="card p-6 fade-up" style={{ animationDelay: '100ms' }}>
            <h3 className="font-display text-lg font-semibold mb-5" style={{ color: 'var(--text-primary)' }}>Category Breakdown</h3>
            <div className="flex flex-col lg:flex-row gap-6 items-center">
              <div className="w-full max-w-[180px] mx-auto lg:mx-0">
                <PieChart width={180} height={180}>
                  ...
                  <Tooltip formatter={(v) => formatCurrency(v)} contentStyle={{ background: '#2d3548', border: '1px solid rgba(61,74,99,0.5)', borderRadius: 12, fontFamily: 'DM Sans' }} />
                </PieChart>
              </div>
              <div className="flex flex-col gap-2 flex-1 min-w-0">
                {categoryData.map(c => (
                  <div key={c.name} className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs font-mono flex-shrink-0" style={{ color: 'var(--text-primary)' }}>{formatCurrency(c.value, true)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="card p-6 fade-up" style={{ animationDelay: '200ms' }}>
            <h3 className="font-display text-lg font-semibold mb-5" style={{ color: 'var(--text-primary)' }}>Monthly Net Savings</h3>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={monthlyData.map(m => ({ month: m.month, savings: m.income - m.expenses }))}>
                ...
                <YAxis tick={{ fill: 'rgba(255,255,255,0.3)', fontSize: 10, fontFamily: 'DM Mono' }} axisLine={false} tickLine={false} tickFormatter={v => `₹${v/1000}k`} />
                <Tooltip formatter={(v) => formatCurrency(v)} contentStyle={{ background: '#2d3548', border: '1px solid rgba(61,74,99,0.5)', borderRadius: 12, fontFamily: 'DM Sans' }} />
                <Bar dataKey="savings" name="Savings" fill="#6dc7a4" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
