...
import { User, Bell, Shield, Palette, CreditCard, HelpCircle, LogOut } from 'lucide-react';
...
const sections = [
  ...
  },
  {
    ...
    toggles: [
      { label: 'Dark Mode', desc: 'Switch between light and dark themes', on: true, key: 'theme' },
    ],
  },
  {
...
export default function Settings({ theme, onToggleTheme, userInfo: globalUserInfo, setUserInfo: setGlobalUserInfo }) {
  ...
  const fetchUserInfo = async () => {
    try {
      ...
      if (data.userInfo) {
        ...
        localStorage.setItem('userInfo', JSON.stringify({ fullName: data.userInfo.fullName || 'User' }));
      } else {
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('userInfo', JSON.stringify({ fullName: userInfo.fullName || 'User' }));
    ...
    <div className="p-8 max-w-2xl mx-auto flex flex-col gap-6">
      ...
      {sections.map((s, si) => (
        ...
        >
            ...
            <div className="flex flex-col gap-4">
              {s.toggles.map(t => (
                <div key={t.label} className="flex items-center justify-between">
                  ...
                  <Toggle on={t.key === 'theme' ? theme === 'dark' : t.on} onClick={t.key === 'theme' ? onToggleTheme : undefined} />
                </div>
              ))}
            </div>
          )}
          ...
          {s.help && (
            <div className="flex flex-col gap-4">
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Having trouble? Send us a message and we'll get back to you.</p>
              <textarea
                value={helpMessage}
          ...
          {s.logout && (
            <div className="flex flex-col gap-4">
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Sign out of your account. You'll need to log in again to access your data.</p>
              <button
          ...
          {s.fields && (
            <div className="mt-5 flex justify-end gap-3">
              {saveMessage && (
                <span className="text-sm" style={{ color: 'var(--success)' }}>{saveMessage}</span>
              )}
              <button
                onClick={handleSave}
                disabled={loading}
